var ctrl_DataGridDossierAtt = app.controller('ctrlDataGridDossierAtt',
  function($rootScope,$scope,$http,$compile) {
    ...
    function successPost(dataJson){
        ...
        var btnhtml = '<jsontabledossieratt data="data"></jsontabledossieratt> ';
        var temp = $compile(btnhtml)($scope);
        angular.element(document.getElementById('datasDossiersAtt')).append(temp);
...
ctrl_DataGridDossierAtt.filter('startFrom', function() {
...
ctrl_DataGridDossierAtt.directive('jsontabledossieratt', function($rootScope,$timeout) {
	return {
		restrict : 'E',
		transclude : true,
		scope : {
		...
		link : function(scope, element, attrs, controllers) {
			...
			try {
				...
				logger(scope.table.head.value,'scope.table.head.value','head');
			} catch(e) {
				// anything
			}
		}
